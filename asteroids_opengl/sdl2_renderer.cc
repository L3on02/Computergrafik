...
void SDL2Renderer::renderSpaceship(vec2 position, float angle) {
...
void SDL2Renderer::render(Saucer * saucer) {
  ...
  vec2 position = saucer->get_position();
  float scale = 0.5;
...
void SDL2Renderer::render(Asteroid * asteroid) {
  ...
  vec2 position = asteroid->get_position();
...
void SDL2Renderer::render(SpaceshipDebris * debris) {
  ...
  static std::array<vec2, 6> debris_direction = { vec2{-40, -23}, vec2{50, 15}, vec2{0, 45},
                                                       vec2{60, -15}, vec2{10, -52}, vec2{-40, 30} };
  vec2 position = debris->get_position();
